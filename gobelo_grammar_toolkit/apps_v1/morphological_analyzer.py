...
class MorphologicalAnalyzer:
    ...
    """
    # inserted new code to prefer HFST lookup if backend is available, with fallback to existing heuristic analysis if HFST returns no results (OOV word)
    def analyze(self, token: str, max_hypotheses: int = 5) -> SegmentedToken:
        if not token or not token.strip():
            raise MorphAnalysisError("token must be non-empty.")
        
        # 🔥 defensive fix
        if not hasattr(self, "_backend"):
            self._backend = None

        # ── HFST path (preferred when backend is available) ──────────
        if self._backend:
            try:
                ...
                print(f"[WARN] HFST failed, falling back: {e}")
        
            # Fall through to heuristic if HFST returns nothing (OOV word)

        # ── Heuristic fallback (existing code, unchanged) ─────────────
        normed = self._normalise(token.strip())
        # existing _analyze_verbal / _analyze_nominal logic
