...
NC8 prefix	zi-	i- ← unique across the entire 7-language ggtk set
NC14 prefix	bu-	bu- (consistent)
...
The most linguistically significant feature is NC8 = i- — no other language in the ggtk set uses this form (Chitonga has zi-, Chibemba bi-, Luvale vi-, Kaonde bi-/fi-, SiLozi li-). This creates a noteworthy homophony with the NC9 subject marker i-, documented in parser notes throughout. All forms carry # VERIFY flags pointing to Kawasha (2003) as the authoritative source.
...
Synthesized Chinyanja linguistic features and resolved negation slot ambiguities
chinyanja.yaml — 3,365 lines, all 18 checks passing. ChiNyanja is by far the most divergent language in the ggtk set. Here's the full divergence summary:
...
NC14 prefix	bu-	u- (unique in ggtk — allomorph w- before V)
...
Immediate (days, mostly plumbing existing ggtk modules)
...
Teachers already print grammar charts and pin them to classroom walls. Give them a tool where they type a verb root (e.g. -bona) and download a print-ready A3 poster showing the full conjugation across all TAM forms, with concord tables. Your ggtk already generates all of this — it's mostly a rendering problem.
...
Architected comprehensive grammar editing interface with nested data management
Here's your ggtk Grammar Admin tool. Here's what it does:

Upload & Download

Drag-and-drop or click to upload any .yaml grammar file
Edits persist to browser storage so you don't lose work on refresh
Download button exports clean YAML back out (same js-yaml format ggtk uses)
...
The ggtk already has all the data in the YAML files. The poster needs to show:
...
Generates conjugation table from ggtk grammar rules
...
Has a print button
Since we're working in React and I can't load the actual YAML files, I'll build with hardcoded Chitonga grammar data as a demonstration, clearly structured so it's easy to swap in real ggtk data.
...
All seven languages are wired in with their correct SC and TAM forms from the ggtk YAML files.
...
Hook the conjugation engine into the ggtk Python backend so the morphophonology rules apply — right now u + aka + bona + a renders as uakabona rather than the correct wakabona via glide formation (SND.1). That one pass through your existing analyser fixes every form across all seven languages simultaneously.

Proceed to provide the code for the poster app and the  the conjugation engine into the ggtk Python backend so the morphophonology rules apply
...
A Python conjugation engine that hooks into the ggtk morphophonology rules (SND.1, SND.2, SND.3, SND.4, CA.1, CA.2, VH.1 etc.)
...
from conjugator.engine import conjugate
from conjugator import GRAMMARS

g  = GRAMMARS['chitonga']
sc = g['subject_concords']
t  = g['tam']

print('=== CHITONGA CONJUGATION TESTS ===')
cases = [
    ('NC1  PRES POS', 'NC1',  'PRES', False, 'wabona'),
    ('NC1  PST  POS', 'NC1',  'PST',  False, 'wakabona'),
    ('NC1  FUT  POS', 'NC1',  'FUT',  False, 'uyobona'),
    ('NC1  PERF POS', 'NC1',  'PERF', False, 'wabonide'),
    ('NC1  SUBJ POS', 'NC1',  'SUBJ', False, 'wabbone'),
    ('3SG  PRES POS', '3SG',  'PRES', False, 'wabona'),
    ('1SG  PRES POS', '1SG',  'PRES', False, 'ndabona'),
    ('3PL  PRES POS', '3PL',  'PRES', False, 'babona'),
    ('NC4  PRES POS', 'NC4',  'PRES', False, 'yabona'),
    ...
    ('NC11 PRES POS', 'NC11', 'PRES', False, 'lwabona'),
    # Negatives - pre-initial ta-
    # ta + SC(u) -> SND.2 a+u->o -> 'to' + stem(bon) + i = 'toboni'
    ('NC1  PRES NEG', 'NC1',  'PRES', True,  'toboni'),
    # ta + SC(nd) -> ta+nd (no sandhi, nd starts with consonant) -> 'tand' + bon + i = 'tandboni'
    ('1SG  PRES NEG', '1SG',  'PRES', True,  'tandboni'),
    # ta + SC(ba) -> ta+ba -> a+b -> no vowel start -> 'taba' + bon + i = 'tababoni'
    ('3PL  PRES NEG', '3PL',  'PRES', True,  'tababoni'),
]
for desc, sc_k, tid, neg, exp in cases:
    got = conjugate(sc[sc_k], t[tid], 'bona', neg,
                    g['neg_type'], g['neg_pre'], g['neg_infix'], 'chitonga')
    ok = got == exp
    print(f'  {\"OK\" if ok else \"FAIL\"} {desc}: {got!r} (exp {exp!r})')
...
tN  = gN['tam']
nyanja = [
    # ChiNyanja: infix negation; no TAM blocked when neg
    # NC1 SM=u, PRES marker=ma, root=bona (stem=bon), fv=a
    # u + ma + bon + a -> join(u,ma) = SND.1 u+m -> 'um' (consonant start, no rule) -> uma
    # join('uma','bon') = 'umbon'? No: uma + bon -> a+b no rule -> 'umbon' + a = 'umbona'
    # Wait: u + ma -> u ends in 'u', ma starts with 'm' (consonant) -> no SND -> 'uma'
    # 'uma' + 'bon' -> 'a'+'b' -> no rule -> 'umbon' -> + 'a' -> 'umbona'? That's wrong!
    # Expected: 'umabona'
    # The issue: the stem stripping. Root='bona', fv='a' (vowel), so stem='bon'
    # parts: ['u','ma','bon','a'] 
    # join('u','ma') = u+m -> no rule -> 'uma'
    # join('uma','bon') = a+b -> no rule -> 'umabon'
    # join('umabon','a') = n+a -> no rule -> 'umabona' ✓
    ('NC1  PRES POS', 'NC1', 'PRES', False, 'umabona'),
    ('NC7  PRES POS', 'NC7', 'PRES', False, 'chimabona'),
    ('NC1  PST  POS', 'NC1', 'PST',  False, 'unabona'),
    ('NC1  FUT  POS', 'NC1', 'FUT',  False, 'udzabona'),
    # NEG: u + sa(infix) + bon + a (ma TAM is blocked by -sa-)
    # parts: ['u','sa','bon','a']
    # join('u','sa') = u+s -> no rule (s is consonant) -> 'usa'
    # join('usa','bon') = a+b -> 'usabon'  join('usabon','a') -> 'usabona' ✓
    ('NC1  PRES NEG', 'NC1', 'PRES', True,  'usabona'),
]
for desc, sc_k, tid, neg, exp in nyanja:
    got = conjugate(scN[sc_k], tN[tid], 'bona', neg,
                    gN['neg_type'], gN['neg_pre'], gN['neg_infix'], 'chinyanja')
    ok = got == exp
    print(f'  {\"OK\" if ok else \"FAIL\"} {desc}: {got!r} (exp {exp!r})')

print()
print('=== VARIETY OF ROOTS ===')
for root, desc in [('lya','eat'),('ya','go'),('bona','see'),('imba','sing'),('lenga','think')]:
    g2 = GRAMMARS['chitonga']
    pos = conjugate(g2['subject_concords']['NC1'], g2['tam']['PRES'], root, False,
                    g2['neg_type'],g2['neg_pre'],g2['neg_infix'],'chitonga')
    neg = conjugate(g2['subject_concords']['NC1'], g2['tam']['PRES'], root, True,
                    g2['neg_type'],g2['neg_pre'],g2['neg_infix'],'chitonga')
    print(f'  -{root}- ({desc}): POS={pos}  NEG={neg}')
"
Output

=== CHITONGA CONJUGATION TESTS ===
  OK NC1  PRES POS: 'wabona' (exp 'wabona')
  OK NC1  PST  POS: 'wakabona' (exp 'wakabona')
  OK NC1  FUT  POS: 'uyobona' (exp 'uyobona')
  OK NC1  PERF POS: 'wabonide' (exp 'wabonide')
  FAIL NC1  SUBJ POS: 'ubone' (exp 'wabbone')
  OK 3SG  PRES POS: 'wabona' (exp 'wabona')
  OK 1SG  PRES POS: 'ndabona' (exp 'ndabona')
  OK 3PL  PRES POS: 'babona' (exp 'babona')
  OK NC4  PRES POS: 'yabona' (exp 'yabona')
  OK NC7  PRES POS: 'cyabona' (exp 'cyabona')
  OK NC11 PRES POS: 'lwabona' (exp 'lwabona')
  FAIL NC1  PRES NEG: 'toaboni' (exp 'toboni')
  FAIL 1SG  PRES NEG: 'tandaboni' (exp 'tandboni')
  OK 3PL  PRES NEG: 'tababoni' (exp 'tababoni')

=== CHINYANJA CONJUGATION TESTS ===
  OK NC1  PRES POS: 'umabona' (exp 'umabona')
  OK NC7  PRES POS: 'chimabona' (exp 'chimabona')
  OK NC1  PST  POS: 'unabona' (exp 'unabona')
  OK NC1  FUT  POS: 'udzabona' (exp 'udzabona')
  FAIL NC1  PRES NEG: 'usamabone' (exp 'usabona')

=== VARIETY OF ROOTS ===
  -lya- (eat): POS=walya  NEG=toalyi
  -ya- (go): POS=waya  NEG=toayi
  -bona- (see): POS=wabona  NEG=toaboni
  -imba- (sing): POS=wemba  NEG=toembi
  -lenga- (think): POS=walenga  NEG=toalengi
...
# With your ggtk YAML files:
ggtk_YAML_DIR=/path/to/yaml python app.py
The load_yaml_grammar() function enriches the embedded data with the full YAML content when the files are present — so any edits made in the ggtk Admin tool flow directly into the poster generator.
