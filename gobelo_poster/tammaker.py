...
cd /home/claude/gobelo_poster && python -c "
# Good: join('ta','u') = 'to' correctly.
# But result was 'toaboni' -- that means 'a' is appearing between 'to' and 'boni'
# The parts for NC1 PRES NEG:
# neg_type='pre', neg_pre='ta', SC='u', marker='a' (PRES), stem='bon', fv='i'
# WAIT -- the TAM marker for PRES is 'a'! So for negatives with pre-initial neg,
# the TAM marker is still included. That's wrong for Chitonga.
# In Chitonga: negative = ta + SC + ROOT + neg_fv (NO TAM marker in simple neg)
# ta-u-boni (not ta-u-a-boni)
# Solution: suppress the TAM marker in negative forms for pre-initial negation.
# OR: the neg construction in Chitonga drops the TAM marker.
# Looking at YAML: 'negation_pre.present.forms = [ta]' and 'negation_pre.past.forms = [tee,tii]'
# The negative present = ta + SC + root + 'i' (no TAM marker 'a')

# Fix: suppress TAM marker when negative AND neg_type == 'pre'
# (The pre-initial NEG replaces the whole tense marking)
print('Parts issue: TAM marker included in negative')
print('Fix: suppress marker when negative and pre-initial')

# Also: ChiNyanja neg 'usamabone': sa is infix, but TAM 'ma' is still there
# ChiNyanja: -sa- REPLACES the TAM marker position (per the YAML: blocks TAM)
# Fix for Nyanja: when neg_type='infix', suppress the TAM marker too
print('ChiNyanja: sa-infix should block TAM marker too')
"
Output

Parts issue: TAM marker included in negative
Fix: suppress marker when negative and pre-initial
ChiNyanja: sa-infix should block TAM marker too
Done
