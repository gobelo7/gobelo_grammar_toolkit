...
def _norm_content_type(ct: str) -> str:
    ...
    ``"override"`` is the v2 content_type produced when a slot is filled
    via ``MorphFeatureBundle.extra_slots``; it is passed through unchanged
    and exempted from ``CONTENT_TYPE_MISMATCH`` checking in Layer 1.
    """
    return {
        ...
        # v2: caller-supplied slot override — not validated against allowed types
        "override":        "override",
    }.get(ct, ct)
...
class VerbSlotValidator:
    ...
    def _check_slot_structure(
        ...
        for a in assignments:
            ...
            # v2: "override" morphemes are intentional caller-supplied values;
            # skip content-type checking — the caller accepts responsibility.
            if norm_ct == "override":
                continue
    ...
    def _check_reference_validity(
        ...
        for a in assignments:
            ...
            # v2: "override" morphemes are caller-supplied; skip all
            # reference-validity checks — the content is intentional.
            if ct == "override":
                continue
