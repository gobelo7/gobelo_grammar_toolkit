...
ISO 639-3 Code:         <ISO_CODE>         # e.g., bem, nya, loz, lun, lue, kqn, mgr
Guthrie Classification: <GUTHRIE_CODE>     # e.g., M.42, N.31, K.21, L.52, K.14, L.41, M.15
Primary Region:         <REGION>
Approximate Speakers:   <NUMBER>
Major Dialects:         <DIALECT_1>, <DIALECT_2>, <DIALECT_3>
Reference Grammar:      <AUTHOR_YEAR_TITLE>
...
# Generated file convention — add language root wrapper:
<language_name>_grammar:
  metadata:
  phonology:
  noun_class_system:
  concord_system:
...
- Root key:   <language_name_lowercase>_grammar:
- File name:  <language_name>_grammar.yaml
```
...
```
STRUCTURAL CHECKS:
[ ] Root key is <language_name>_grammar: (not chitonga_grammar:)
[ ] 6 top-level keys under root: metadata, phonology, noun_class_system,
...
with open('<language_name>_grammar.yaml', 'r', encoding='utf-8') as f:
    grammar = yaml.safe_load(f)

root_key = list(grammar.keys())[0]
assert root_key == '<language_name>_grammar', f"Wrong root key: {root_key}"
...
lang = grammar[list(grammar.keys())[0]]
...
with open('chitonga.yaml', 'r') as f:
    chitonga = yaml.safe_load(f)   # flat root — no language wrapper in chitonga.yaml
...
ck = get_deep_keys(chitonga)        # chitonga keys (no wrapper)
nk = get_deep_keys(lang)            # lang already unwrapped
