...
export default function ExampleViewer({ ncKey }) {
  ...
  return (
    <div className="bg-ggtk-card border border-ggtk-border rounded-lg p-4">
      ...
      <div className="text-xs font-mono text-ggtk-text mb-2">
        Prefix: <span className="text-ggtk-accent font-bold">{nc.prefix?.canonical_form ?? "—"}</span>
      </div>
      {referents.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-2">
          {referents.map((r, i) => (
            <span key={i} className="bg-ggtk-input border border-ggtk-border rounded px-2 py-0.5 font-sans text-[10px] text-ggtk-muted">
