...
export default function Sidebar() {
  ...
  return (
    <nav className="w-[172px] bg-ggtk-panel border-r border-ggtk-border py-3.5 shrink-0 flex flex-col">
      {nav.map(item => {
        ...
        return (
          <button
            ...
            className={`flex items-center gap-2.5 px-4 py-2.5 border-none bg-transparent cursor-pointer text-left font-sans text-[13px] transition-all border-r-2 ${
              activeView === item.id
                ? "text-ggtk-accent font-bold border-r-ggtk-accent"
                : "text-ggtk-muted font-normal border-r-transparent hover:text-ggtk-text"
            }`}
          >
            <span className={`font-mono text-[10px] font-bold w-3.5 ${
              item.warn && vCount > 0 ? "text-ggtk-verify" : activeView === item.id ? "text-ggtk-accent" : "text-ggtk-border"
            }`}>{item.icon}</span>
            {label}
          </button>
        );
      ...
      {grammar && (
        <div className="mt-auto pt-3.5 px-4 border-t border-ggtk-border">
          <div className="text-[9px] text-ggtk-muted tracking-[0.1em] uppercase font-sans mb-1.5">Stats</div>
          {[
            ["NC Classes",    ncCount],
            ...
            <div key={label} className="flex justify-between text-[11px] mb-0.5">
              <span className="text-ggtk-muted font-sans">{label}</span>
              <span className={`font-mono ${label.includes("VERIFY") && val > 0 ? "text-ggtk-verify" : "text-ggtk-text"}`}>{val}</span>
            </div>
          ))}
        </div>
      )}
