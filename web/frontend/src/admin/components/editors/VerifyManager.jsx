  ...
  return (
    <div>
      ...
      <div className="mb-6 pb-3.5 border-b border-ggtk-border flex items-start justify-between">
        <div>
          <h2 className="m-0 text-ggtk-text font-sans font-extrabold text-lg">VERIFY Flags</h2>
        ...
        <div className="flex items-center gap-3">
          <span className={`font-mono text-sm font-bold ${verifyCount > 0 ? "text-ggtk-verify" : "text-ggtk-success"}`}>
      ...
      {!loading && suggestions.length === 0 && (
        <div className="text-center py-14">
          ...
          <div className="text-ggtk-success font-sans text-sm">
        ...
        <>
          <div className="mb-4">
            <input value={search} onChange={e => setSearch(e.target.value)}
              ...
              className="bg-ggtk-input border border-ggtk-border rounded text-ggtk-text font-sans text-xs px-3 py-2 outline-none w-96 max-w-full focus:border-ggtk-accent"/>
          </div>

          <div className="flex flex-col gap-2">
            {filtered.map(item => (
              <div key={item.id} className="bg-ggtk-card border-l-4 border-l-ggtk-verify border border-ggtk-border rounded-lg px-4 py-3">
                <div className="flex items-center gap-2 mb-1.5">
                  <span className={`text-[9px] font-mono font-bold px-1.5 py-0.5 rounded ${
                    item.type === "verify_flag" ? "bg-ggtk-verifyBg text-ggtk-verify" : "bg-ggtk-danger/10 text-ggtk-danger"
                  }`}>
                    {item.type === "verify_flag" ? "VERIFY" : "SCHEMA"}
                  </span>
                  <span className="font-mono text-[9px] text-ggtk-muted break-all">{item.path}</span>
                </div>
                ...
                {editing === item.id
                  ? (
                    <textarea value={drafts[item.id] ?? ""} autoFocus rows={3}
                      ...
                      className="w-full bg-ggtk-input border border-ggtk-border rounded font-mono text-xs text-ggtk-text px-2.5 py-1.5 outline-none resize-y mb-2"/>
                  )
                  : (
                    <div className="font-mono text-xs text-ggtk-verify bg-ggtk-verifyBg px-2.5 py-1.5 rounded leading-relaxed mb-2">
