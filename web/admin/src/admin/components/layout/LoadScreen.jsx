...
export default function LoadScreen() {
  ...
  return (
    <div
      className="min-h-screen bg-ggtk-bg flex items-center justify-center font-sans"
    ...
    >
      <div className={`text-center px-14 py-12 border-2 rounded-2xl max-w-md transition-all duration-200 ${
        drag
          ? "border-ggtk-accent bg-ggtk-accentBg"
          : "border-ggtk-border bg-transparent"
      }`}>
        <div className="text-5xl mb-5">⌘</div>
        <h1 className="text-ggtk-text font-extrabold text-[22px] m-0 mb-1.5 tracking-tight">
          GGT Grammar Admin
        </h1>
        <p className="text-ggtk-muted text-xs m-0 mb-2.5 tracking-[0.04em] uppercase">
          Gobelo Grammar Toolkit — v2
        </p>
        <p className="text-ggtk-muted text-[13px] mb-8 leading-relaxed">
          Drop a <code className="text-ggtk-accent font-mono">.yaml</code> grammar file
          or click to upload.
          <br />
          <span className="text-[11px]">Grammar lives in memory only — re-upload on refresh.</span>
        </p>

        <label className="inline-block px-8 py-3 rounded-lg bg-ggtk-accent text-white font-extrabold text-[13px] cursor-pointer tracking-[0.06em] shadow-[0_4px_24px_rgba(232,147,74,0.3)] hover:opacity-90 transition-opacity">
          Upload YAML
